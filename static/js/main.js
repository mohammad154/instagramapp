...
function addOrRemoveLike(post_id) {
    let thumb = document.getElementById('thumbs_' + post_id);
    let local_likes = document.getElementById("postlikes_" + post_id);
...
function showComments(post_id) {
    let section = document.getElementById('comments-for-post-' + post_id);
    section.textContent = "";
    ...
    $.ajax({
        ...
        success: function (data) {
            data.forEach(element => {
                ...
                let commenter = document.createElement('h6');
...
function uploadComment(post_id, fname, lname) {
    ...
    let data = document.getElementById('comment-field-' + post_id).value;
    if (data === "") {
        return;
    }
...
function followHandler(person_id) {
    let button = document.querySelector('#follow-person-' + person_id);
