...
from user.models import Following, Person
from post.models import Post, Liked, Comment
...
def get_user_like_status(person_id, post_id):
                ...
                post_modified.append(
                    {
                        ...
                        'text': post.text,
                        'likes': post.likes,
                        'user_liked': get_user_like_status(person.id, post.id),
...
def remove_like(request):
    ...
    Liked.objects.filter(post_id=post_id, person_id=person_id).update(liked=0)
        ...
        post_modified.append(
            {
                ...
                'text': post.text,
                'likes': post.likes,
                'user_liked': get_user_like_status(active.id, post.id),
                'comment_count': get_comment_count(post.id),
            }
        )
